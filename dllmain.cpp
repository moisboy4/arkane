...
#include "includes/arkane/discord_rpc.h"
...
bool unload( HMODULE module ) {
    ...
    printf( ( "arkane | unloading hooks\n" ) );
    sdk::hooks::unload( );
    printf( ( "arkane | unloaded hooks\n" ) );

    std::this_thread::sleep_for( std::chrono::milliseconds( 500 ) );

    printf( ( "arkane | unloading\n" ) );
...
DWORD WINAPI on_attach(void* instance) {
	[&]() {
        ...
        interfaces::console->console_color_printf({ 255, 0, 0, 255 }, ("[arkane] "));
        interfaces::console->console_printf(std::string("successfully injected").append(" \n").c_str());

        if (interfaces::engine->is_in_game()) {
            interfaces::chat_element->chatprintf("#arkane#_print_injected");
        }
	}();
